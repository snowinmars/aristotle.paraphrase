...
import {getTheme, exportTheme} from "./ColorSettings.helpers";
...
const ColorSettings: FunctionComponent = () => {
  ...
  return (
    <Container fluid className={styles.prfColorSettings}>
      <Row>
        <Col>
          <ButtonGroup className={styles.prfColorSettingsGroup}>
            {colors.map((color) => (
              <ToggleButton
                key={color.id}
                id={`radio-${color.id}`}
                className={styles.prfColorSettingsItem}
                type="radio"
                variant="secondary"
                name="radio"
